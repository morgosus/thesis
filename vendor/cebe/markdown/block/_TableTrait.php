        ...
        for ($i = $current, $count = count($lines); $i < $count; $i++) {
            ...
            if ($i == $current+1) {
                ...
                foreach($cols as $col) {
                    ...
                    $r = (substr($col, -1, 1) === ':');
                    if ($l && $r) {
                        $block['cols'][] = 'table-align-center';
                    } elseif ($l) {
                        $block['cols'][] = 'table-align-left';
                    } elseif ($r) {
                        $block['cols'][] = 'table-align-right';
                    } else {
                        $block['cols'][] = '';
                    }
                }
    ...
    protected function renderTable($block)
    {
        ...
        foreach($block['rows'] as $row) {
            ...
            foreach ($row as $c => $cell) {
                $align = empty($cols[$c]) ? '' : ' class="' . $cols[$c] . '"';
                $tds .= "<$cellTag$align><div>" . trim($this->renderAbsy($cell)) . "</div></$cellTag>";
            }
            if ($first) {
                $head .= "<tr>$tds</tr>\n";
            } else {
