...
class MarkdownExtra extends Markdown
{
	...
	protected function renderHeadline($block)
	{
		...
		$tag = 'h' . $block['level'];
		$attributes = $this->renderAttributes($block);
		return "<$tag$attributes class=\"markdown__$tag\">" . rtrim($this->renderAbsy($block['content']), "# \t") . "</$tag>\n";
	}
	...
	protected function renderImage($block)
	{
		...
		$attributes = $this->renderAttributes($block);
		return '<img class="markdown__img" src="' . htmlspecialchars($block['url'], ENT_COMPAT | ENT_HTML401, 'UTF-8') . '"'
